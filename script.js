(function () {
  ...
  function startVoiceControl() {
    ...
    voiceRecognition.addEventListener("result", (event) => {
      ...
      if (isThrowCommand(transcript)) showFoldedCard();
    });
  ...
  function isThrowCommand(transcript) {
    const clean = transcript.replace(/[^a-z\s]/g, " ").replace(/\s+/g, " ").trim();
    if (!clean) return false;

    const words = clean.split(" ");
    return words.some((word) => {
      if (["throw", "through", "thro", "threw", "tero", "tro"].includes(word)) return true;
      if (word.length < 3 || word.length > 7) return false;

      const target = "throw";
      let distance = Math.abs(word.length - target.length);
      for (let i = 0; i < Math.min(word.length, target.length); i += 1) {
        if (word[i] !== target[i]) distance += 1;
      }
      return distance <= 2;
    });
  }
  ...
  function showFoldedCard() {
    ...
    foldedCard.style.setProperty("--card-scale", ".9");
